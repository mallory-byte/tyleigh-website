...
(function () {
  ...
  function headerHTML() {
    ...
    return '' +
      '<header class="site-header">' +
        '<div class="nav">' +
          '<a class="wordmark" href="index.html">' + S.name.replace(' Family Farms', '') +
            '<small>' + S.town.toLowerCase() + ', mississippi</small></a>' +
  ...
  function footerHTML() {
    var strip = [S.images.pasture1, S.images.highland1, S.images.calf1, S.images.field1]
      .map(function (u) { return '<div class="s" style="' + bg(u) + '"></div>'; }).join('');
    return '' +
      '<footer class="site-footer">' +
        '<div class="foot-strip">' + strip + '</div>' +
        '<div class="wrap foot-main">' +
          '<div>' +
            '<div class="wordmark">' + S.name.replace(' Family Farms', '') +
              '<small>' + S.town.toLowerCase() + ', mississippi</small></div>' +
            '<p class="muted" style="margin-top:1rem;max-width:34ch;font-size:.95rem">' + esc(S.tagline) + '</p>' +
          '</div>' +
          '<div class="foot-col">' +
            '<h4>the farm</h4>' +
            '<ul>' +
  ...
  function animalCard(a) {
    var line = a.story.split('. ')[0];
    if (line.length > 96) line = line.slice(0, 93) + '…'; else line += '.';
    ...
    return '' +
      '<a class="animal-card" href="animal.html?a=' + a.slug + '">' +
        tag +
        '<div class="photo" style="' + bg(a.photos[0]) + '"></div>' +
        '<div class="cap">' +
  ...
  function renderFeaturedFold() {
    ...
    var feat = ANIMALS.filter(function (a) { return a.featured; }).slice(0, 4);
    host.innerHTML = feat.map(animalCard).join('');
  }

  function renderFold() {
    var host = byId('foldGroups'); if (!host) return;
    var groups = [
      ['herd sire', 'Our herd sire', function (a) { return a.status === 'Herd Sire' || (a.sex === 'Bull'); }],
      ['the cows', 'The cows', function (a) { return a.sex === 'Cow'; }],
      ['heifers & calves', 'Heifers & young ones', function (a) { return a.sex === 'Heifer' || a.sex === 'Calf' || a.sex === 'Steer'; }]
    ];
    var used = {};
    var html = '';
    groups.forEach(function (g) {
      var members = ANIMALS.filter(function (a) {
        ...
        // exclude sold from the living fold view
  ...
  function renderAnimalDetail() {
    ...
    var barn = a.barnName ? ' <span style="font-style:italic;color:var(--brown);font-size:.6em"> &nbsp;“' + esc(a.barnName) + '”</span>' : '';

    var thumbs = a.photos.map(function (p, i) {
      return '<button class="thumb' + (i === 0 ? ' active' : '') + '" data-src="' + p + '"><span class="t" style="' + bg(p) + '"></span></button>';
    }).join('');
    ...
    var sc = statusClass(a.status);
    var tag = sc ? '<span class="status-tag ' + sc + '" style="position:static;display:inline-block;margin-bottom:1rem">' + esc(a.status.toLowerCase()) + '</span>' : '<span class="label">' + esc(a.status.toLowerCase()) + '</span>';

    host.innerHTML = '' +
      '<div class="wrap section">' +
        '<a class="label" href="fold.html" style="display:inline-block;margin-bottom:2rem">&larr; our fold</a>' +
        '<div class="detail-grid">' +
          '<div>' +
            '<div class="gallery-main" id="galMain" style="' + bg(a.photos[0]) + '"></div>' +
            '<div class="gallery-thumbs" id="galThumbs">' + thumbs + '</div>' +
          '</div>' +
          '<div>' +
            tag +
            '<h1 class="display" style="font-size:clamp(2.4rem,6vw,3.6rem)">' + esc(a.name) + barn + '</h1>' +
            '<div class="meta" style="margin:.6rem 0 1.6rem;color:var(--moss);letter-spacing:.1em;text-transform:lowercase;font-size:.85rem">' +
            ...
            '<table class="spec-table">' +
              ...
              '<tr><th>dam</th><td>' + esc(a.dam) + (a.dam && animalDamLink(a.dam)) + '</td></tr>' +
    ...
    // gallery interaction
    var main = byId('galMain'), thumbEls = host.querySelectorAll('.thumb');
  ...
  function animalDamLink() { return ''; } // placeholder hook; dams here are external lines

  function renderJournalList() {
    var host = byId('journalList'); if (!host) return;
    host.innerHTML = JOURNAL.map(function (p) {
      return '<a class="journal-item" href="post.html?p=' + p.slug + '">' +
        '<div class="photo" style="' + bg(p.photo) + '"></div>' +
        '<div>' +
          '<div class="date">' + fmtDate(p.date) + '</div>' +
          '<h3>' + esc(p.title) + '</h3>' +
          '<p>' + esc(p.excerpt) + '</p>' +
          '<span class="textlink" style="display:inline-block;margin-top:.8rem;font-size:.9rem">keep reading &rarr;</span>' +
        '</div>' +
      '</a>';
    }).join('');
  }
  ...
  function renderJournalPreview() {
    var host = byId('journalPreview'); if (!host) return;
    var p = JOURNAL[0];
    host.innerHTML =
      '<a class="journal-item" href="post.html?p=' + p.slug + '" style="border-bottom:0;padding-bottom:0">' +
        '<div class="photo" style="' + bg(p.photo) + '"></div>' +
        '<div>' +
          '<div class="date">' + fmtDate(p.date) + '</div>' +
          '<h3>' + esc(p.title) + '</h3>' +
  ...
  function renderPost() {
    ...
    host.innerHTML =
      '<div class="wrap narrow section">' +
        '<a class="label" href="journal.html" style="display:inline-block;margin-bottom:1.5rem">&larr; ' + esc(S.journalName.toLowerCase()) + '</a>' +
        '<div class="date" style="color:var(--moss);letter-spacing:.14em;text-transform:lowercase;font-size:.78rem">' + fmtDate(p.date) + '</div>' +
        '<h1 class="page-title" style="margin:.5rem 0 2rem">' + esc(p.title) + '</h1>' +
        '<div class="imgband" style="' + bg(p.photo) + ';margin-bottom:2.5rem"></div>' +
  ...
  function renderCountdown() {
    document.querySelectorAll('[data-countdown]').forEach(function (host) {
      ...
      host.innerHTML =
        ...
        '<div><div class="num">' + S.calvingYearLabel.slice(2) + '</div><div class="u">' + fmtDateShort(S.nextCalving).toLowerCase() + '</div></div>';
    });
  }
  ...
  function initForm() {
    ...
    // prefill "interested in" from ?about=
    var about = qs('about');
    var sel = form.querySelector('select[name="interest"]');
    if (about && sel) {
      // add a specific-animal option if it matches an animal
    ...
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      // Self-contained: confirm warmly. (Swap for Airtable/email POST when connected.)
  ...
  function fillTokens() {
    ...
    document.querySelectorAll('[data-hero]').forEach(function (n) { n.style.backgroundImage = 'url(\'' + S.images[n.getAttribute('data-hero')] + '\')'; });
    document.querySelectorAll('[data-img]').forEach(function (n) { n.style.backgroundImage = 'url(\'' + S.images[n.getAttribute('data-img')] + '\')'; });
  }
